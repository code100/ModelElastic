...
class ModelElastic extends Component implements EntityInterface, ModelElasticInterface{
	...
	/**
	 * getId: overwrite getId, for zephir, let method = "get" . camelize(name);
	 ...
	 * save data
	 * $user = new users();
	 * $saveData = ['username'=>'abc','email'=>'ddd@gmail.com'];
     ...
     *              'fields'     => $fields,    
	 * 						]);
	 * @todo  NEED MORE TEST by 2016/07/05
	 * @param  array  $opt [description]
	 * @return [type]      [description]
	 */
	...
	public static function cloneResult($model, $document){
		...
		/**
		if (method_exists($clonedModel, 'afterFetch')){
			$clonedModel->afterFetch();
		}**/
	...
	protected static function _getResultset($opt, $model, $unique){
		...
		$maxPage  = isset($opt['max_page'])    ? intval($opt['max_page']) : 100;
